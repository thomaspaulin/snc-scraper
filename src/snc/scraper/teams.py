...
def parse_team(team_elem, division, known_teams):
    ...
    return Team(name=name, division=division, logo_url=logo_url, record=record)


def parse_division(header_elem, known_divisions):
...
def parse(soup, known_teams, known_divisions):
    ...
    for elem in team_elems:
        if len(elem.select('font')) is not 0:
            ...
            parsed_teams.append(parse_team(elem, current_division, known_teams))
    return parsed_teams
